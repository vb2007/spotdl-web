...
| v21 | `dev-release-automation` | **In progress.** Versioning contract (backend/frontend, CI-enforced) + GitHub releases with generated changelogs + public GHCR container images + automated pull-and-restart deploy to the Debian host, with a CI gate, pre-migration backup, and health-gated rollback. Inserted ahead of hardening — see `docs/RELEASE_PIPELINE.md` |
