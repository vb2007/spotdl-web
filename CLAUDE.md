...
| v18 | `dev-job-centric-api` | **Done.** Cursor-paginated/filtered/sorted/searchable `/api/jobs` and `/api/tracks`; rollup status + title computed in one aggregate query; `pg_trgm` search; unpaginated `list_tracks` removed |
